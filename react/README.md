...
###  2. Vercel 배포 환경에서 404 Page 미동작 및 새로고침 에러 (SPA Routing Issue)

* **문제 현상 (Issue)**:
  - 잘못된 경로(예: `/invalid-path`)로 접속 시, React Router에서 설정한 `NotFoundPage` 컴포넌트 대신 **Vercel 자체 404 에러 페이지**가 출력됨.
  - 메인 페이지 외의 상세 페이지(`/items/123`) 등에서 브라우저 **새로고침(F5)** 시 404 에러 발생.

* **원인 분석 (Cause)**:
  - React는 싱글 페이지 애플리케이션(SPA) 구조로, 하나의 `index.html` 내에서 클라이언트 사이드 라우팅(Client-side Routing)을 수행합니다.
  - 하지만 배포 서버(Vercel)는 사용자의 URL 요청이 올 때마다 정적 파일을 실제 서버 경로에서 찾으려 시도하므로, 해당 물리적 파일이 없어 404 에러를 반환하는 백엔드 서버 라우팅 특성 때문이었습니다.

* **해결 방법 (Solution)**:
  - 루트 경로에 `vercel.json` 설정 파일을 작성하여, 모든 와일드카드 경로(`/(.*)`)의 HTTP 요청을 앱 진입점인 `/index.html`로 우회(Rewrite)하도록 처리했습니다.
  - 이를 통해 모든 라우팅 권한을 클라이언트(React Router)에게 위임함으로써, **새로고침 문제 해결 및 잘못된 접근 시 커스텀 `NotFoundPage` 컴포넌트가 정상 렌더링**되도록 보완했습니다.
